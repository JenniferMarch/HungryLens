...
T1<-Sys.time()
addmJAGS_want <- jags.parallel(ddmData,inits = NULL,jags.seed = sample(100:999,1),
...
ddmRhats <- addmJAGS_want$BUGSoutput$summary[,8] #check with max(Rhats), which should ideally be < 1.01 (1.05 would also be okay)
max(ddmRhats)
addmJAGS_want$BUGSoutput$DIC
#max R_hat = 1.025; DIC = 68933
save.image(file='maaDDM_want.RData')
```

## 2.2maaDDM_2w: check parameters (group posteriors)
```{r}
#get group parameter estimates in transformed form 
boundSep_S <- log(1+exp(addmJAGS_want$BUGSoutput$sims.list$mu_bound))
boundSep_H <- log(1+exp(addmJAGS_want$BUGSoutput$sims.list$mu_bound+addmJAGS_want$BUGSoutput$sims.list$mu_bound_H))
boundSep_delta <- addmJAGS_want$BUGSoutput$sims.list$mu_bound_H

ndt_S <- log(1+exp(addmJAGS_want$BUGSoutput$sims.list$mu_ndt))
ndt_H <- log(1+exp(addmJAGS_want$BUGSoutput$sims.list$mu_ndt+addmJAGS_want$BUGSoutput$sims.list$mu_ndt_H))
ndt_delta <- addmJAGS_want$BUGSoutput$sims.list$mu_ndt_H

weight1_S <- pnorm(addmJAGS_want$BUGSoutput$sims.list$mu_weight1)
weight1_H <- pnorm(addmJAGS_want$BUGSoutput$sims.list$mu_weight1+addmJAGS_want$BUGSoutput$sims.list$mu_weight1_H)
weight1_delta <- addmJAGS_want$BUGSoutput$sims.list$mu_weight1_H

drift_S <- log(1+exp(addmJAGS_want$BUGSoutput$sims.list$mu_drift))
drift_H <- log(1+exp(addmJAGS_want$BUGSoutput$sims.list$mu_drift+addmJAGS_want$BUGSoutput$sims.list$mu_drift_H))
drift_delta <- addmJAGS_want$BUGSoutput$sims.list$mu_drift_H

theta_S <- (addmJAGS_want$BUGSoutput$sims.list$mu_theta)
theta_H <- (addmJAGS_want$BUGSoutput$sims.list$mu_theta+addmJAGS_want$BUGSoutput$sims.list$mu_theta_H)
theta_delta <- addmJAGS_want$BUGSoutput$sims.list$mu_theta_H

phy_S <- (addmJAGS_want$BUGSoutput$sims.list$mu_phy)
phy_H <- (addmJAGS_want$BUGSoutput$sims.list$mu_phy+addmJAGS_want$BUGSoutput$sims.list$mu_phy_H)
phy_delta <- addmJAGS_want$BUGSoutput$sims.list$mu_phy_H
...
ggsave("combined_plot_maaddm_want.png", combined_plot_maaddm, width = 16, height = 12)
