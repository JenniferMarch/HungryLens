...
T1<-Sys.time()
ddmJAGS_nutri <- jags.parallel(ddmData,inits = NULL,
...
ddmRhats <- ddmJAGS_nutri$BUGSoutput$summary[,8] #check with max(Rhats), which should ideally be < 1.01 (1.05 would also be okay)
max(ddmRhats)
ddmJAGS_nutri$BUGSoutput$DIC

save.image(file="workspace_DDM_nutri.RData")
```
## 2.2 maDDM_2w: check parameters (group posteriors)
```{r}
#get group parameter estimates in transformed form 
boundSep_S <- log(1+exp(ddmJAGS_nutri$BUGSoutput$sims.list$mu_bound))
boundSep_H <- log(1+exp(ddmJAGS_nutri$BUGSoutput$sims.list$mu_bound+ddmJAGS_nutri$BUGSoutput$sims.list$mu_bound_H))
boundSep_delta <- ddmJAGS_nutri$BUGSoutput$sims.list$mu_bound_H

ndt_S <- log(1+exp(ddmJAGS_nutri$BUGSoutput$sims.list$mu_ndt))
ndt_H <- log(1+exp(ddmJAGS_nutri$BUGSoutput$sims.list$mu_ndt+ddmJAGS_nutri$BUGSoutput$sims.list$mu_ndt_H))
ndt_delta <- ddmJAGS_nutri$BUGSoutput$sims.list$mu_ndt_H

weight1_S <- pnorm(ddmJAGS_nutri$BUGSoutput$sims.list$mu_weight1)
weight1_H <- pnorm(ddmJAGS_nutri$BUGSoutput$sims.list$mu_weight1+ddmJAGS_nutri$BUGSoutput$sims.list$mu_weight1_H)
weight1_delta <- ddmJAGS_nutri$BUGSoutput$sims.list$mu_weight1_H

drift_S <- log(1+exp(ddmJAGS_nutri$BUGSoutput$sims.list$mu_drift))
drift_H <- log(1+exp(ddmJAGS_nutri$BUGSoutput$sims.list$mu_drift+ddmJAGS_nutri$BUGSoutput$sims.list$mu_drift_H))
drift_delta <- ddmJAGS_nutri$BUGSoutput$sims.list$mu_drift_H
...
ggsave("combined_plot_ddm1_nutri.png", combined_plot_ddm1, width = 16, height = 12)
```
