...
T1<-Sys.time()
addmJAGS_sp_nutri<- jags.parallel(ddmData,inits = NULL,#jags.seed = sample(100:999,1),
...
# check convergence
ddmRhats <- addmJAGS_sp_nutri$BUGSoutput$summary[,8] #check with max(Rhats), which should ideally be < 1.01 (1.05 would also be okay)
max(ddmRhats)
addmJAGS_sp_nutri$BUGSoutput$DIC

# Recommended to save addmJAGS_sp_nutri
save.image("maaDDMsp_nutri.RData")
```

## 2.2.maaDDM_sp: check parameters (group posteriors)
```{r}
#get group parameter estimates in transformed form 
boundSep_S <- log(1+exp(addmJAGS_sp_nutri$BUGSoutput$sims.list$mu_bound))
boundSep_H <- log(1+exp(addmJAGS_sp_nutri$BUGSoutput$sims.list$mu_bound+addmJAGS_sp_nutri$BUGSoutput$sims.list$mu_bound_H))
boundSep_delta <- addmJAGS_sp_nutri$BUGSoutput$sims.list$mu_bound_H

ndt_S <- log(1+exp(addmJAGS_sp_nutri$BUGSoutput$sims.list$mu_ndt))
ndt_H <- log(1+exp(addmJAGS_sp_nutri$BUGSoutput$sims.list$mu_ndt+addmJAGS_sp_nutri$BUGSoutput$sims.list$mu_ndt_H))
ndt_delta <- addmJAGS_sp_nutri$BUGSoutput$sims.list$mu_ndt_H

weight1_S <- pnorm(addmJAGS_sp_nutri$BUGSoutput$sims.list$mu_weight1)
weight1_H <- pnorm(addmJAGS_sp_nutri$BUGSoutput$sims.list$mu_weight1+addmJAGS_sp_nutri$BUGSoutput$sims.list$mu_weight1_H)
weight1_delta <- addmJAGS_sp_nutri$BUGSoutput$sims.list$mu_weight1_H

drift_S <- log(1+exp(addmJAGS_sp_nutri$BUGSoutput$sims.list$mu_drift))
drift_H <- log(1+exp(addmJAGS_sp_nutri$BUGSoutput$sims.list$mu_drift+addmJAGS_sp_nutri$BUGSoutput$sims.list$mu_drift_H))
drift_delta <- addmJAGS_sp_nutri$BUGSoutput$sims.list$mu_drift_H

sp_S <- pnorm(addmJAGS_sp_nutri$BUGSoutput$sims.list$mu_sp)
sp_H <- pnorm(addmJAGS_sp_nutri$BUGSoutput$sims.list$mu_sp+addmJAGS_sp_nutri$BUGSoutput$sims.list$mu_sp_H)
sp_delta <- addmJAGS_sp_nutri$BUGSoutput$sims.list$mu_sp_H

theta_S <- (addmJAGS_sp_nutri$BUGSoutput$sims.list$mu_theta)
theta_H <- (addmJAGS_sp_nutri$BUGSoutput$sims.list$mu_theta+addmJAGS_sp_nutri$BUGSoutput$sims.list$mu_theta_H)
theta_delta <- addmJAGS_sp_nutri$BUGSoutput$sims.list$mu_theta_H

phy_S <- (addmJAGS_sp_nutri$BUGSoutput$sims.list$mu_phy)
phy_H <- (addmJAGS_sp_nutri$BUGSoutput$sims.list$mu_phy+addmJAGS_sp_nutri$BUGSoutput$sims.list$mu_phy_H)
phy_delta <- addmJAGS_sp_nutri$BUGSoutput$sims.list$mu_phy_H
...
ggsave("combined_plot_maaddm2_nutri.png", combined_plot_maaddm2, width = 16, height = 12)
```
