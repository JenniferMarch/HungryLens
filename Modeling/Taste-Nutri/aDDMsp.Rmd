...
T1<-Sys.time()
addmJAGS_sp_noPhi <- jags.parallel(ddmData,inits = NULL,#jags.seed = sample(100:999,1),
...
# check convergence
ddmRhats <- addmJAGS_sp_noPhi$BUGSoutput$summary[,8] #check with max(Rhats), which should ideally be < 1.01 (1.05 would also be okay)
max(ddmRhats)
addmJAGS_sp_noPhi$BUGSoutput$DIC

save.image(file='workspace_addm_sp_nutri.RData')
...
boundSep_S <- log(1+exp(addmJAGS_sp_noPhi$BUGSoutput$sims.list$mu_bound))
boundSep_H <- log(1+exp(addmJAGS_sp_noPhi$BUGSoutput$sims.list$mu_bound+addmJAGS_sp_noPhi$BUGSoutput$sims.list$mu_bound_H))
boundSep_delta <- addmJAGS_sp_noPhi$BUGSoutput$sims.list$mu_bound_H

ndt_S <- log(1+exp(addmJAGS_sp_noPhi$BUGSoutput$sims.list$mu_ndt))
ndt_H <- log(1+exp(addmJAGS_sp_noPhi$BUGSoutput$sims.list$mu_ndt+addmJAGS_sp_noPhi$BUGSoutput$sims.list$mu_ndt_H))
ndt_delta <- addmJAGS_sp_noPhi$BUGSoutput$sims.list$mu_ndt_H

weight1_S <- pnorm(addmJAGS_sp_noPhi$BUGSoutput$sims.list$mu_weight1)
weight1_H <- pnorm(addmJAGS_sp_noPhi$BUGSoutput$sims.list$mu_weight1+addmJAGS_sp_noPhi$BUGSoutput$sims.list$mu_weight1_H)
weight1_delta <- addmJAGS_sp_noPhi$BUGSoutput$sims.list$mu_weight1_H

drift_S <- log(1+exp(addmJAGS_sp_noPhi$BUGSoutput$sims.list$mu_drift))
drift_H <- log(1+exp(addmJAGS_sp_noPhi$BUGSoutput$sims.list$mu_drift+addmJAGS_sp_noPhi$BUGSoutput$sims.list$mu_drift_H))
drift_delta <- addmJAGS_sp_noPhi$BUGSoutput$sims.list$mu_drift_H

theta_S <- pnorm(addmJAGS_sp_noPhi$BUGSoutput$sims.list$mu_theta)
theta_H <- pnorm(addmJAGS_sp_noPhi$BUGSoutput$sims.list$mu_theta+addmJAGS_sp_noPhi$BUGSoutput$sims.list$mu_theta_H)
theta_delta <- addmJAGS_sp_noPhi$BUGSoutput$sims.list$mu_theta_H

sp_S <- pnorm(addmJAGS_sp_noPhi$BUGSoutput$sims.list$mu_sp)
sp_H <- pnorm(addmJAGS_sp_noPhi$BUGSoutput$sims.list$mu_sp+addmJAGS_sp_noPhi$BUGSoutput$sims.list$mu_sp_H)
sp_delta <- addmJAGS_sp_noPhi$BUGSoutput$sims.list$mu_sp_H
...
combined_plot_addm2 <- plot_grid(tasteweight1, tasteweight2, drift1, drift2, nDT1, nDT2, bound1, bound2, theta1, theta2, startpoint1, startpoint2, labels = c("a", "", "b", "", "c", "", "d", "", "e", "", "f", ""), label_size = 20, ncol = 4, align = "h", column_spacing = unit(c(1, 3, 1, 1, 3, 1, 1, 3, 1), "cm"))
combined_plot_addm2
# Save the combined plot as a PNG file
ggsave("combined_plot_addm2_nutri.png", combined_plot_addm2, width = 16, height = 12)
```