...
T1<-Sys.time()
addmJAGS <- jags.parallel(ddmData,inits = NULL,jags.seed = sample(100:999,1),
...
ddmRhats <- addmJAGS$BUGSoutput$summary[,8] #check with max(Rhats), which should ideally be < 1.01 (1.05 would also be okay)
max(ddmRhats)
addmJAGS$BUGSoutput$DIC
#max R_hat = 1.025; DIC = 68933
save.image(file='maaDDM.RData')
```

## 2.2maaDDM_2w: check parameters (group posteriors)
```{r}
#get group parameter estimates in transformed form 
boundSep_S <- log(1+exp(addmJAGS$BUGSoutput$sims.list$mu_bound))
boundSep_H <- log(1+exp(addmJAGS$BUGSoutput$sims.list$mu_bound+addmJAGS$BUGSoutput$sims.list$mu_bound_H))
boundSep_delta <- addmJAGS$BUGSoutput$sims.list$mu_bound_H

ndt_S <- log(1+exp(addmJAGS$BUGSoutput$sims.list$mu_ndt))
ndt_H <- log(1+exp(addmJAGS$BUGSoutput$sims.list$mu_ndt+addmJAGS$BUGSoutput$sims.list$mu_ndt_H))
ndt_delta <- addmJAGS$BUGSoutput$sims.list$mu_ndt_H

weight1_S <- pnorm(addmJAGS$BUGSoutput$sims.list$mu_weight1)
weight1_H <- pnorm(addmJAGS$BUGSoutput$sims.list$mu_weight1+addmJAGS$BUGSoutput$sims.list$mu_weight1_H)
weight1_delta <- addmJAGS$BUGSoutput$sims.list$mu_weight1_H

drift_S <- log(1+exp(addmJAGS$BUGSoutput$sims.list$mu_drift))
drift_H <- log(1+exp(addmJAGS$BUGSoutput$sims.list$mu_drift+addmJAGS$BUGSoutput$sims.list$mu_drift_H))
drift_delta <- addmJAGS$BUGSoutput$sims.list$mu_drift_H

theta_S <- (addmJAGS$BUGSoutput$sims.list$mu_theta)
theta_H <- (addmJAGS$BUGSoutput$sims.list$mu_theta+addmJAGS$BUGSoutput$sims.list$mu_theta_H)
theta_delta <- addmJAGS$BUGSoutput$sims.list$mu_theta_H

phy_S <- (addmJAGS$BUGSoutput$sims.list$mu_phy)
phy_H <- (addmJAGS$BUGSoutput$sims.list$mu_phy+addmJAGS$BUGSoutput$sims.list$mu_phy_H)
phy_delta <- addmJAGS$BUGSoutput$sims.list$mu_phy_H
...
ggsave("combined_plot_maaddm.png", combined_plot_maaddm, width = 16, height = 12)
...
sim_addmJAGS  <- function(parameters,tasteA,tasteB,healthA,healthB,fixProps){
...
nPostPred <- 1000
ppSample <- sample(addmJAGS$BUGSoutput$n.sims,nPostPred) #draw samples from posterior (without replacement)
  ...
  for  (s in (unique(data_sated$subject))){
    ...
    for (t in 1:2){
      if (t == 1){ #session-specific stuff up front; note that order of parameters follows rtdists, not dwiener
        ...
        parameters <- c(log(1+exp(addmJAGS$BUGSoutput$sims.list$bound[x,sInJAGS]+addmJAGS$BUGSoutput$sims.list$bound_H[x,sInJAGS])),
                      log(1+exp(addmJAGS$BUGSoutput$sims.list$ndt[x,sInJAGS]+addmJAGS$BUGSoutput$sims.list$ndt_H[x,sInJAGS])),
                      log(1+exp(addmJAGS$BUGSoutput$sims.list$drift[x,sInJAGS]+addmJAGS$BUGSoutput$sims.list$drift_H[x,sInJAGS])),
                      pnorm(addmJAGS$BUGSoutput$sims.list$weight1[x,sInJAGS]+addmJAGS$BUGSoutput$sims.list$weight1_H[x,sInJAGS]),
                      (addmJAGS$BUGSoutput$sims.list$theta[x,sInJAGS]+addmJAGS$BUGSoutput$sims.list$theta_H[x,sInJAGS]),
                      (addmJAGS$BUGSoutput$sims.list$phy[x,sInJAGS]+addmJAGS$BUGSoutput$sims.list$phy_H[x,sInJAGS]))
      } else {
        sub <- data_sated[data_sated$subject == s, ] #sated
        fixatedElement <- expanded_table_s[[which(unique(data_sated$subject)==s)]]
        parameters <- c(log(1+exp(addmJAGS$BUGSoutput$sims.list$bound[x,sInJAGS])),
                      log(1+exp(addmJAGS$BUGSoutput$sims.list$ndt[x,sInJAGS])),
                      log(1+exp(addmJAGS$BUGSoutput$sims.list$drift[x,sInJAGS])),
                      pnorm(addmJAGS$BUGSoutput$sims.list$weight1[x,sInJAGS]),
                      (addmJAGS$BUGSoutput$sims.list$theta[x,sInJAGS]),
                      (addmJAGS$BUGSoutput$sims.list$phy[x,sInJAGS]))
      }
      ...
      #simulate
      simDat <- sim_addmJAGS(parameters,tasteAi,tasteBi,healthAi,healthBi,fixProps)
      if (t==1){
