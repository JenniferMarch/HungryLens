...
ddmData <- list('N'=N,'S'=S,'P'=P,'H'=H,'tasteA'=tasteA,'tasteB'=tasteB,'healthA'=healthA,'healthB'=healthB,'RT'=RT,'RT2'=abs(RT))
...
T1<-Sys.time()
ddmJAGS_rst <- jags.parallel(ddmData,inits = NULL,#jags.seed = sample(100:999,1),
...
# check convergence
ddmRhats <- ddmJAGS_rst$BUGSoutput$summary[,8] #check with max(Rhats), which should ideally be < 1.01 (1.05 would also be okay)
max(ddmRhats) # 1.056
ddmJAGS_rst$BUGSoutput$DIC # 69138.18
#Recommended to save ddmJAGS_rst
save.image("ddmJAGS_rst.RData")

```

## 2.2 maDDM_rst: check parameters (group posteriors)
```{r}
boundSep_S <- log(1+exp(ddmJAGS_rst$BUGSoutput$sims.list$mu_bound))
boundSep_H <- log(1+exp(ddmJAGS_rst$BUGSoutput$sims.list$mu_bound+ddmJAGS_rst$BUGSoutput$sims.list$mu_bound_H))
boundSep_delta <- ddmJAGS_rst$BUGSoutput$sims.list$mu_bound_H

ndt_S <- log(1+exp(ddmJAGS_rst$BUGSoutput$sims.list$mu_ndt))
ndt_H <- log(1+exp(ddmJAGS_rst$BUGSoutput$sims.list$mu_ndt+ddmJAGS_rst$BUGSoutput$sims.list$mu_ndt_H))
ndt_delta <- ddmJAGS_rst$BUGSoutput$sims.list$mu_ndt_H

weight1_S <- pnorm(ddmJAGS_rst$BUGSoutput$sims.list$mu_weight1)
weight1_H <- pnorm(ddmJAGS_rst$BUGSoutput$sims.list$mu_weight1+ddmJAGS_rst$BUGSoutput$sims.list$mu_weight1_H)
weight1_delta <- ddmJAGS_rst$BUGSoutput$sims.list$mu_weight1_H

drift_S <- log(1+exp(ddmJAGS_rst$BUGSoutput$sims.list$mu_drift))
drift_H <- log(1+exp(ddmJAGS_rst$BUGSoutput$sims.list$mu_drift+ddmJAGS_rst$BUGSoutput$sims.list$mu_drift_H))
drift_delta <- ddmJAGS_rst$BUGSoutput$sims.list$mu_drift_H

rst_S <- ddmJAGS_rst$BUGSoutput$sims.list$mu_rst
rst_H <- ddmJAGS_rst$BUGSoutput$sims.list$mu_rst+ddmJAGS_rst$BUGSoutput$sims.list$mu_rst_H
rst_delta <- ddmJAGS_rst$BUGSoutput$sims.list$mu_rst_H
