...
T1<-Sys.time()
addmJAGS_sp<- jags.parallel(ddmData,inits = NULL,#jags.seed = sample(100:999,1),
...
# check convergence
ddmRhats <- addmJAGS_sp$BUGSoutput$summary[,8] #check with max(Rhats), which should ideally be < 1.01 (1.05 would also be okay)
max(ddmRhats)
addmJAGS_sp$BUGSoutput$DIC

# Recommended to save addmJAGS_sp
save.image(file='workspace_maaddm_sp.RData')
```

## 2.2.maaDDM_sp: check parameters (group posteriors)
```{r}
#get group parameter estimates in transformed form 
boundSep_S <- log(1+exp(addmJAGS_sp$BUGSoutput$sims.list$mu_bound))
boundSep_H <- log(1+exp(addmJAGS_sp$BUGSoutput$sims.list$mu_bound+addmJAGS_sp$BUGSoutput$sims.list$mu_bound_H))
boundSep_delta <- addmJAGS_sp$BUGSoutput$sims.list$mu_bound_H

ndt_S <- log(1+exp(addmJAGS_sp$BUGSoutput$sims.list$mu_ndt))
ndt_H <- log(1+exp(addmJAGS_sp$BUGSoutput$sims.list$mu_ndt+addmJAGS_sp$BUGSoutput$sims.list$mu_ndt_H))
ndt_delta <- addmJAGS_sp$BUGSoutput$sims.list$mu_ndt_H

weight1_S <- pnorm(addmJAGS_sp$BUGSoutput$sims.list$mu_weight1)
weight1_H <- pnorm(addmJAGS_sp$BUGSoutput$sims.list$mu_weight1+addmJAGS_sp$BUGSoutput$sims.list$mu_weight1_H)
weight1_delta <- addmJAGS_sp$BUGSoutput$sims.list$mu_weight1_H

drift_S <- log(1+exp(addmJAGS_sp$BUGSoutput$sims.list$mu_drift))
drift_H <- log(1+exp(addmJAGS_sp$BUGSoutput$sims.list$mu_drift+addmJAGS_sp$BUGSoutput$sims.list$mu_drift_H))
drift_delta <- addmJAGS_sp$BUGSoutput$sims.list$mu_drift_H

sp_S <- pnorm(addmJAGS_sp$BUGSoutput$sims.list$mu_sp)
sp_H <- pnorm(addmJAGS_sp$BUGSoutput$sims.list$mu_sp+addmJAGS_sp$BUGSoutput$sims.list$mu_sp_H)
sp_delta <- addmJAGS_sp$BUGSoutput$sims.list$mu_sp_H

theta_S <- (addmJAGS_sp$BUGSoutput$sims.list$mu_theta)
theta_H <- (addmJAGS_sp$BUGSoutput$sims.list$mu_theta+addmJAGS_sp$BUGSoutput$sims.list$mu_theta_H)
theta_delta <- addmJAGS_sp$BUGSoutput$sims.list$mu_theta_H

phy_S <- (addmJAGS_sp$BUGSoutput$sims.list$mu_phy)
phy_H <- (addmJAGS_sp$BUGSoutput$sims.list$mu_phy+addmJAGS_sp$BUGSoutput$sims.list$mu_phy_H)
phy_delta <- addmJAGS_sp$BUGSoutput$sims.list$mu_phy_H
...
startpoint2<-ggplot(data_deltasp, aes(x = sp_delta)) +
  geom_histogram(binwidth = 1/240, fill = "grey", alpha = 0.5, color = "black") +
  geom_vline(xintercept = 0, color = "red", size = 1.5) +
...
theta1<-ggplot(datatheta, aes(x = theta_S)) +
  geom_histogram(binwidth = 1/90, fill = "cornflowerblue", alpha = 0.5, color = "black") +
  geom_histogram(aes(x = theta_H), binwidth = 1/90, fill = "gold", alpha = 0.5, color = "black") +
...
theta2<-ggplot(data_deltatheta, aes(x = theta_delta)) +
  geom_histogram(binwidth = 1/65, fill = "grey", alpha = 0.5, color = "black") +
  geom_vline(xintercept = 0, color = "red", size = 1.5) +
...
phy1<-ggplot(dataphy, aes(x = phy_S)) +
  geom_histogram(binwidth = 1/30, fill = "cornflowerblue", alpha = 0.5, color = "black") +
  geom_histogram(aes(x = phy_H), binwidth = 1/30, fill = "gold", alpha = 0.5, color = "black") +
...
combined_plot_maaddm2 <- plot_grid(tasteweight1, tasteweight2, drift1, drift2, nDT1, nDT2, bound1, bound2, theta1, theta2, phy1, phy2, startpoint1, startpoint2, labels = c("a", "", "b", "", "c", "", "d", "", "e", "", "f", "", "g", ""), label_size = 20, ncol = 4, align = "h", column_spacing = unit(c(1, 3, 1, 1, 3, 1, 1, 3, 1,1), "cm"))
combined_plot_maaddm2
# Save the combined plot as a PNG file
ggsave("combined_plot_maaddm2.png", combined_plot_maaddm2, width = 16, height = 12)
```
...
sim_addmJAGS_sp  <- function(parameters,tasteA,tasteB,healthA,healthB,fixProps){
  ...
  valueDifference <- fixProps[,1]*(weight*(tasteA-theta*tasteB)+(1-weight)*phi*(healthA-theta*healthB))+
                     fixProps[,2]*(weight*(theta*tasteA-tasteB)+(1-weight)*phi*(theta*healthA-healthB))+
                     fixProps[,3]*(weight*phi*(tasteA-theta*tasteB)+(1-weight)*(healthA-theta*healthB))+
                     fixProps[,4]*(weight*phi*(theta*tasteA-tasteB)+(1-weight)*(theta*healthA-healthB))
...
nPostPred <- 1000
ppSample <- sample(addmJAGS_sp$BUGSoutput$n.sims,nPostPred) #draw samples from posterior (without replacement)
  ...
  for  (s in (unique(data_sated$subject))){
    ...
    for (t in 1:2){
      if (t == 1){ #session-specific stuff up front; note that order of parameters follows rtdists, not dwiener
        ...
        parameters <- c(log(1+exp(addmJAGS_sp$BUGSoutput$sims.list$bound[x,sInJAGS]+addmJAGS_sp$BUGSoutput$sims.list$bound_H[x,sInJAGS])),
                      log(1+exp(addmJAGS_sp$BUGSoutput$sims.list$ndt[x,sInJAGS]+addmJAGS_sp$BUGSoutput$sims.list$ndt_H[x,sInJAGS])),
                      log(1+exp(addmJAGS_sp$BUGSoutput$sims.list$drift[x,sInJAGS]+addmJAGS_sp$BUGSoutput$sims.list$drift_H[x,sInJAGS])),
                      pnorm(addmJAGS_sp$BUGSoutput$sims.list$weight1[x,sInJAGS]+addmJAGS_sp$BUGSoutput$sims.list$weight1_H[x,sInJAGS]),
                      (addmJAGS_sp$BUGSoutput$sims.list$theta[x,sInJAGS]+addmJAGS_sp$BUGSoutput$sims.list$theta_H[x,sInJAGS]),
                      (addmJAGS_sp$BUGSoutput$sims.list$phy[x,sInJAGS]+addmJAGS_sp$BUGSoutput$sims.list$phy_H[x,sInJAGS]),
                      pnorm(addmJAGS_sp$BUGSoutput$sims.list$sp[x,sInJAGS]+addmJAGS_sp$BUGSoutput$sims.list$sp_H[x,sInJAGS]))
      } else {
        sub <- data_sated[data_sated$subject == s, ] #sated
        fixatedElement <- expanded_table_s[[which(unique(data_sated$subject)==s)]]
        parameters <- c(log(1+exp(addmJAGS_sp$BUGSoutput$sims.list$bound[x,sInJAGS])),
                      log(1+exp(addmJAGS_sp$BUGSoutput$sims.list$ndt[x,sInJAGS])),
                      log(1+exp(addmJAGS_sp$BUGSoutput$sims.list$drift[x,sInJAGS])),
                      pnorm(addmJAGS_sp$BUGSoutput$sims.list$weight1[x,sInJAGS]),
                      (addmJAGS_sp$BUGSoutput$sims.list$theta[x,sInJAGS]),
                      (addmJAGS_sp$BUGSoutput$sims.list$phy[x,sInJAGS]),
                      pnorm(addmJAGS_sp$BUGSoutput$sims.list$sp[x,sInJAGS]))
      }
      ...
      #simulate
      simDat <- sim_addmJAGS_sp(parameters,tasteAi,tasteBi,healthAi,healthBi,fixProps)
      if (t==1){
